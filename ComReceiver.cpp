...
void sendAnswer(Communication *output,char function,char address,char job,char const *answer,uint8_t noerror)
{
//char tempString[36];
	LED_ROT_ON;
...
//	sprintf(tempString,"%c%c%c%s%c",function,address_KNET,job,answer,sign);
	output->send(answer,quelle_KNET,sign,function,address,job,dataType);
	LED_ROT_OFF;
}

void sendPureAnswer(Communication *output,char function,char address,char job,uint8_t noerror)
{
	sendAnswer(output,function,address,job,"",noerror);
}

void sendAnswerInt(Communication *output,char function,char address,char job,uint32_t wert,uint8_t noerror)
{
char str[15];
	sprintf(str,"%lu",wert);
	sendAnswer(output,function,address,job,str,noerror);
}

void sendAnswerDouble(Communication *output,char function,char address,char job,double wert,uint8_t noerror)
{
char temp[20];
	sprintf(temp,"%lf",wert);
	sendAnswer(output,function,address,job,temp,noerror);
}
...
void doJob(Communication *output)
{
	if (  (rec_state_KNET == RCST_DO_JOB) && (job_KNET > 0) )
	{
		...
		{
			 sendAnswer(output,commands[job_KNET-1].function,address_KNET,commands[job_KNET-1].job,fehler_text[SECURITY_ERROR],false);
		}
		else
		{
